...
const fakeSnapshot = () => ({
  ...
  modules: { cpu: { ok: true, usage: 10 + (tick++ % 50) }, memory: { ok: true, usagePct: 40 } }
});
...
const check = setInterval(async () => {
  ...
  if (receivedConfig && receivedConfig.logLevel === 'warn' && receivedConfig.pushIntervalMs === 1000) {
    ...
    if (saved.logLevel === 'warn' && saved.pushIntervalMs === 1000 && saved.modules && saved.modules.cpu === true && saved.modules.disks === false) {
      console.log('REMOTE CONFIG applied by slave:', JSON.stringify(receivedConfig));
      console.log('TEST PASSED (master pushes remote config, slave applies it)');
      clearInterval(check);
